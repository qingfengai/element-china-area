...
ORIGINAL_DATA.map((item, index) => {
  REGION_DATA['86'][item.value + '0000'] = item.label
  REGION_DATA[item.value + '0000'] = {}

  item.children.map((items, indexs) => {
    REGION_DATA[item.value + '0000'][items.value + '00'] = items.label
    REGION_DATA[items.value + '00'] = {}
    items.children.map((itemss, indexss) => {
      REGION_DATA[items.value + '00'][itemss.value] = itemss.label
    })
